...
function WorldCommandMap({nations}:{nations:(HomeNation&{rank?:number;isCurrent:boolean})[]}){
 ...
 return <div className="world-command-map"><svg ref={svg} viewBox="0 0 1000 500" role="img" aria-label="World map showing leading Diplomatia nations" onPointerDown={event=>{if(camera.zoom>1){drag.current={clientX:event.clientX,clientY:event.clientY,x:camera.x,y:camera.y};event.currentTarget.setPointerCapture(event.pointerId)}}} onPointerMove={event=>{if(!drag.current||!svg.current)return;const rect=svg.current.getBoundingClientRect();setCamera(clamp({...camera,x:drag.current.x+(event.clientX-drag.current.clientX)*1000/rect.width,y:drag.current.y+(event.clientY-drag.current.clientY)*500/rect.height}))}} onPointerUp={()=>{drag.current=null}} onPointerCancel={()=>{drag.current=null}}>
  <rect width="1000" height="500" className="command-map-ocean"/><g transform={`translate(${camera.x} ${camera.y}) scale(${camera.zoom})`}><image className="command-map-land" href="/world-map.svg" width="1000" height="500" preserveAspectRatio="none"/><WorldTraffic/>{positioned.map(item=><g className={`command-nation-marker${item.isCurrent?' current':''}`} transform={`translate(${item.x} ${item.y}) scale(${1/camera.zoom})`} role="link" tabIndex={0} aria-label={`${item.name}, ${Number(item.population).toLocaleString()} population`} onClick={()=>navigate(`/nation/${encodeURIComponent(item.id)}`)} onKeyDown={event=>{if(event.key==='Enter'||event.key===' ')navigate(`/nation/${encodeURIComponent(item.id)}`)}} key={item.id}><circle className="marker-halo" r="15"/><circle className="marker-core" r="8"/><text y="3">{item.rank||'•'}</text><g className="marker-label" transform="translate(13 -13)"><rect width={Math.max(82,item.name.length*7+28)} height="31" rx="2"/><text x="9" y="13">{item.name}</text><text className="marker-population" x="9" y="25">{compact(item.population)} population{item.approximate?' · approximate':''}</text></g><title>{item.name} · {Number(item.population).toLocaleString()} population</title></g>)}</g>
