...
<Shield/>
<div>
<span className="eyebrow">NATIONAL DEFENSE INVENTORY</span>
<h2>Military Command</h2>
<p>Produce forces, manage mobilization and capacity, and maintain the units committed to active campaigns.</p>
</div>
</section>
  {!data.projectRequirementsEnabled&&<p className="notice military-testing-notice">
<b>Testing configuration:</b> National Project requirements for domestic military production are temporarily disabled.</p>}
  {error&&<p className="error notice">{error}</p>}
  <div className="military-summary">
<div>
<Users/>
<span>Population</span>
<b>{Number(data.population).toLocaleString()}</b>
</div>
<div>
<Anchor/>
<span>Provinces</span>
<b>{Number(data.provinces).toLocaleString()}</b>
</div>
<div>
<Coins/>
<span>Daily cash upkeep</span>
<b>{yen(dailyCash)}</b>
</div>
<div>
<Zap/>
...
<header>
<div>
<span className="eyebrow">EXPEDITIONARY LOGISTICS</span>
<h3>Forward Operating Base</h3>
</div>
<MapPinned/>
</header>
<div className="fob-facts">
<span>
<b>{fobs.count} / {fobs.maximum}</b> operational</span>
<span>
<b>¥{Number(fobs.cost).toLocaleString()}</b> construction</span>
<span>
<b>{fobs.provinceRequirement} Provinces</b> required</span>
</div>{fobs.bases.length?fobs.bases.map((base:any)=>
<article className="fob-existing" key={base.id}>
<div>
<b>{base.name}</b>
<small>{base.continent} · established {new Date(base.createdAt).toLocaleDateString()}</small>
</div>
<button className="danger" disabled={busy!==''} onClick={()=>void demolishFob(base.id)}>
<Trash2/>Demolish</button>
</article>):<>{fobs.rebuildAfter&&new Date(fobs.rebuildAfter)>new Date()?<p className="notice">Construction locked until {new Date(fobs.rebuildAfter).toLocaleDateString()}.</p>:Number(fobs.provinces)<Number(fobs.provinceRequirement)?<p className="notice">Forward Operating Base construction unlocks at {fobs.provinceRequirement} Provinces.</p>:<>
<WorldLocationPicker value={fobLocation} onChange={setFobLocation} mode="fob"/>
<button className="primary" disabled={!fobLocation||busy!==''} onClick={()=>void buildFob()}>{busy==='fob'?'Constructing…':'Build Forward Operating Base · ¥25,000,000'}</button>
</>}</>}</section>
  <section className="military-grid">{data.units.map((unit:any)=>
<article className="panel military-unit" key={unit.key}>
   <header>
<div>
<span className="eyebrow">{unit.tradable?'TRADEABLE EQUIPMENT':'DOMESTIC PERSONNEL'}</span>
<h3>{unit.name}</h3>
</div>
<b>{Number(unit.quantity).toLocaleString()} <small>/ {Number(unit.capacity).toLocaleString()}</small>
</b>
</header>
...
<button className="danger" disabled={unit.decommissionLocked||Number(unit.availableQuantity)<=0||busy!==''} onClick={()=>act(unit.key,'decommission')}>
<Trash2/>{busy===unit.key+'decommission'?'Decommissioning…':'Decommission'}</button>
</div>
