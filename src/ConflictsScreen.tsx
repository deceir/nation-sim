...
import './two-front-conflicts.css';
import './conflict-drone.css';

const units=['soldiers','tanks','ships','jets'];
const unitNames:Record<string,string>={soldiers:'Soldiers',tanks:'Tanks',ships:'Ships',jets:'Fighter Jets'};
...
function ConflictDetail({id}:{id:string}){
 ...
 return <div className="conflict-detail-page">
  ...
  <section className="conflict-detail-hero"><div><span className={`war-stage ${data.stage}`}>{data.stage==='ended'?label(data.outcome||'concluded'):label(data.stage)}</span><h2>{data.attacker.name} <small>versus</small> {data.defender.name}</h2><p>{data.objectiveName} · Declared {dateTime(data.declaredAt)}</p><small className="war-objective-effect">{data.objectiveEffect}</small></div><Crosshair/></section>
  <section className="conflict-side-by-side"><ConflictSide title="Declaring nation" nation={data.attacker}/><div className="conflict-center-mark"><Swords/><b>{number(data.attacker.score,1)} - {number(data.defender.score,1)}</b><span>Score</span></div><ConflictSide title="Responding nation" nation={data.defender}/></section>
  ...
  <WarTheaterMap attacker={{name:data.attacker.name,lat:data.attacker.lat,lng:data.attacker.lng,fobs:data.attacker.fobs||[]}} defender={{name:data.defender.name,lat:data.defender.lat,lng:data.defender.lng,fobs:data.defender.fobs||[]}} routeType={data.routeType} stage={data.stage} distanceKm={data.distanceKm} roundsResolved={data.roundsResolved} deployments={data.deployments||[]}/>
  <section className="conflict-detail-grid">
   <section className="panel conflict-forces"><span className="eyebrow">WAR STATISTICS</span><h3>Military commitments</h3><div className="conflict-force-legend"><span>H Homeland</span><span>E Expeditionary</span><span>T In transit</span></div><div className="conflict-force-table"><header><span>Unit</span><b>{data.attacker.name}</b><b>{data.defender.name}</b></header>{units.map(unit=><div key={unit}><span>{unitNames[unit]}</span><ForceValue force={data.forces.attacker?.[unit]}/><ForceValue force={data.forces.defender?.[unit]}/></div>)}</div></section>
   <section className="panel conflict-timeline"><div className="conflict-timeline-title"><div><span className="eyebrow">WAR TIMELINE</span><h3>Strategic reports</h3></div><Activity/></div><div className="conflict-timeline-scroll">{data.droneStrikes?.map((strike:any)=><article className="conflict-drone-event" key={strike.id}><header><b>{data.droneMissions?.[strike.mission]?.name||label(strike.mission)}</b><time>{dateTime(strike.launchedAt)}</time></header><p>{strike.summary}</p></article>)}{data.reports.length?data.reports.map((report:any)=><article key={report.round}><header><b>Round {report.round}</b><time>{dateTime(report.resolvedAt)}</time></header><p>{report.summary}</p><div><span>{data.attacker.name}: H {label(report.attackerHomeOperation)} / {label(report.attackerHomePosture)} · E {label(report.attackerOperation)} / {label(report.attackerForeignPosture)} · {number(report.attackerStrength)} strength</span><span>{data.defender.name}: H {label(report.defenderHomeOperation)} / {label(report.defenderHomePosture)} · E {label(report.defenderOperation)} / {label(report.defenderForeignPosture)} · {number(report.defenderStrength)} strength</span></div><small>Losses: {lossText(report.attackerLosses)} / {lossText(report.defenderLosses)}</small></article>):!data.droneStrikes?.length&&<p className="conflict-no-reports">No strategic reports yet.</p>}</div></section>
  </section>
  <WarDamageSummary ended={data.stage==='ended'} attacker={{name:data.attacker.name,forces:data.forces.attacker||{},incomingPressure:data.attacker.damagePressureReceived,infrastructureDamage:data.attacker.infrastructureDamage,institutionsDestroyed:data.attacker.institutionsDestroyed}} defender={{name:data.defender.name,forces:data.forces.defender||{},incomingPressure:data.defender.damagePressureReceived,infrastructureDamage:data.defender.infrastructureDamage,institutionsDestroyed:data.defender.institutionsDestroyed}}/>
 </div>
}

function ConflictSide({title,nation}:{title:string;nation:any}){return <article className="conflict-side"><span className="eyebrow">{title}</span><NationIdentity nation={nation}/><div className="conflict-side-stats"><span>Resolve <b>{number(nation.resolve)}%</b></span><span>Readiness <b>{number(nation.readiness)}%</b></span><span>Organization <b>{number(nation.organization)}%</b></span><span>Pressure inflicted <b>{number(nation.damagePressureInflicted,2)}</b></span></div></article>}
function ForceValue({force}:{force:any}){return <b className="conflict-front-counts"><span>H {number(force?.homeTheater)}</span><span>E {number(force?.foreignTheater)}</span><span>T {number(force?.enRoute)}</span><small>{number(force?.lost)} lost</small></b>}
