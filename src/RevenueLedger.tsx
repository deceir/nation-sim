...
export default function RevenueLedger({economy,strategy}:{economy:any;strategy:any}){
 ...
 return <section className="panel national-ledger">
  ...
  <div className="ledger-section"><h3>Cash income</h3><CashRow label="Gross citizen tax revenue" daily={citizenIncome} tone="positive"/><CashRow label="Luxury Consumption Income" daily={luxuryIncome} tone="positive"/><CashRow label="Total cash income" daily={citizenIncome+luxuryIncome} tone="subtotal"/></div>
  <div className="ledger-section"><h3>Cash expenses</h3><CashRow label="Infrastructure upkeep" daily={-infrastructureUpkeep} tone="negative"/><CashRow label="Military upkeep" daily={-militaryUpkeep} tone="negative"/><CashRow label={economy.alliance?.name?`${economy.alliance.name} Alliance tax`:'Alliance tax'} daily={-allianceTax} tone="negative"/><CashRow label="Total cash expenses" daily={-(infrastructureUpkeep+militaryUpkeep+allianceTax)} tone="subtotal"/></div>
  <div className="ledger-section"><h3>Resource usage</h3>{usedResources.length?usedResources.map(resource=><ResourceRow key={resource} resource={resource} daily={-(usage[resource]||0)} tone="negative" detail={resource==='foodstuffs'?`${amount(result.dailyFoodConsumption)} population upkeep plus ${amount((usage.foodstuffs||0)-Number(result.dailyFoodConsumption||0))} industrial input`:resource==='energy'?`${amount(economy.military?.dailyEnergyUpkeep||0)} military upkeep plus industrial inputs`:resource==='luxury_goods'?'Optional national Luxury Consumption':'Projected manufacturing input'}/>):<p className="ledger-empty">No resource consumption projected.</p>}</div>
  <div className="ledger-section"><h3>Resource production</h3>{producedResources.length?producedResources.map(resource=><ResourceRow key={resource} resource={resource} daily={production[resource]} tone="positive"/>):<p className="ledger-empty">No commodity production projected.</p>}</div>
  <div className="ledger-section net"><h3>Net national flows</h3><CashRow label="Treasury" daily={netCash} tone={netCash>=0?'positive':'negative'}/>{resources.map(resource=><ResourceRow key={resource} resource={resource} daily={Number(netResources[resource])} tone={Number(netResources[resource])>=0?'positive':'negative'}/>)}</div>
