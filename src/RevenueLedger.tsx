...
export default function RevenueLedger({economy,strategy}:{economy:any;strategy:any}){
 ...
 usage.foodstuffs=Number((result.projectedDailyTotalFoodDemand??result.dailyFoodConsumption)||0);
 ...
 return <section className="panel national-ledger">
  ...
  <details className="ledger-section" open><summary>Resource usage</summary>{usedResources.length?usedResources.map(resource=><ResourceRow key={resource} resource={resource} daily={-(usage[resource]||0)} tone="negative" detail={resource==='foodstuffs'?`${amount(result.dailyCivilianFoodConsumption)} civilian demand (${Number(result.foodDemandSizeMultiplier||1).toFixed(2)}× size scaling), ${amount(result.dailyMilitaryFoodConsumption)} standing-army rations, ${amount(result.projectedDailyWarFoodConsumption)} projected wartime rations, plus ${amount((usage.foodstuffs||0)-Number((result.projectedDailyTotalFoodDemand??result.dailyFoodConsumption)||0))} industrial input`:resource==='energy'?`${amount(economy.military?.dailyEnergyUpkeep||0)} military upkeep plus industrial inputs`:resource==='luxury_goods'?'Optional national Luxury Consumption':'Projected manufacturing input'}/>):<p className="ledger-empty">No resource consumption projected.</p>}</details>
