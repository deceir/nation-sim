...
export default function VenturesScreen(){
 ...
 return <div className="ventures-page">
  ...
  <section className="venture-summary">
   <article><span>Personal Capital</span><b>{yen(a.personalCapital)}</b><small>Deposit capacity {yen(Math.max(0,a.personalCapitalCap-a.personalCapital))}</small></article>
   <article><span>Daily transfer allowance</span><b>{yen(a.transferRemaining)}</b><small>{yen(a.transferUsedToday)} of {yen(a.dailyTransferLimit)} used</small></article>
   <article><span>Capital at work</span><b>{yen(a.activeCapital)}</b><small>{yen(a.activeCapitalCap)} maximum</small></article>
  ...
  <section className="venture-transfer panel">
   ...
   <form onSubmit={submitTransfer}>
    ...
    <label>Amount in Yen<input type="number" min="1" step="1" max={a.transferRemaining} value={transfer} onChange={e=>setTransfer(e.target.value)} placeholder="0"/></label>
    <div className="transfer-balances"><span>National Treasury <b>{yen(a.treasury)}</b></span><span>Personal Capital <b>{yen(a.personalCapital)}</b></span></div>
  ...
  {active.length===0?<section className="panel venture-empty"><BriefcaseBusiness/><h3>No capital currently committed</h3><p>Select an opportunity below when you are ready.</p></section>:<div className="active-venture-grid">{active.map(v=>{const result=(v.payout||0)-v.amountInvested;return <article className={'panel active-venture '+v.status} key={v.id}><header><span className={'risk '+v.risk}>{v.risk} risk</span><span>{v.status==='claimable'?'Matured':remaining(v.maturesAt,now)}</span></header><h3>{v.title}</h3><p>{v.description}</p><div className="venture-position-value"><span>Invested<b>{yen(v.amountInvested)}</b></span>{v.status==='claimable'&&<><span>Result<b className={result>=0?'positive':'negative'}>{result>=0?'+':''}{yen(result)}</b></span><span>Collect<b>{yen(v.payout||0)}</b></span></>}</div>{v.status==='claimable'?<button className="primary" disabled={busy!==''} onClick={()=>act(v.id,'/ventures/'+v.id+'/collect')}>{busy===v.id?'Collecting…':'Collect result'}</button>:<button className="venture-cancel" disabled={busy!==''} onClick={()=>confirm(`Cancel ${v.title}? You will receive ${yen(Math.floor(v.amountInvested*.75))}, equal to 75% of the invested capital.`)&&act(v.id,'/ventures/'+v.id+'/cancel')}>{busy===v.id?'Cancelling…':'Cancel early — return 75%'}</button>}</article>})}</div>}
