...
const unitKeys=['soldiers','tanks','ships','jets'];
...
export function WarRulesPanel({rules}:{rules:any}){
 ...
 return <details className="panel war-rules-panel"><summary><span><BookOpen/><b>Victory conditions and combat modifiers</b></span><small>How campaigns end and what reduces combat strength</small></summary><div className="war-rules-content">
  <section><h4>How a war is won</h4><ul><li>Successful expeditionary operations in the opposing homeland build score, reduce enemy Resolve, and create strategic damage pressure. Defending that homeland denies those gains.</li><li>Every conventional unit not committed abroad defends the homeland in every active war. Sending units abroad reduces that shared defense until they return.</li><li>Reducing enemy Resolve to 0 ends the war immediately in a decisive victory.</li><li>Otherwise, the campaign ends after {r.maximumRounds||20} rounds. A score lead of more than {r.stalemateScoreMargin??3} wins; a closer result is a stalemate.</li><li>Winning margins below {r.majorVictoryMargin||15} are minor, {r.majorVictoryMargin||15}–{(r.decisiveVictoryMargin||35)-1} are major, and {r.decisiveVictoryMargin||35}+ are decisive.</li><li>Capitulation unlocks after round {r.minimumCapitulationRound||4}, or sooner when Resolve falls to {r.earlyCapitulationResolve||35}% or lower.</li></ul></section>
  ...
  <section><h4>Strategic damage and drones</h4><p>Winning control in enemy territory builds damage pressure. Infrastructure and institution losses are assessed when the war ends, capped at {r.maximumInfrastructureDamagePercent||6}% Infrastructure and a {r.maximumInstitutionRiskPercent||3}% destruction chance per institution. Drones operate outside strategic rounds through Drone Command: they can gather intelligence, strike deployed forces, or add limited damage pressure, but never contribute conventional combat strength.</p></section>
 </div></details>
}
