...
type Deployment={id:string;side:'attacker'|'defender';status:'in_transit'|'preparing'|'deployed'|'concluded';arrivesRound:number;departedAt:string;arrivalAt?:string|null;originType?:string;originName?:string;originLat?:number;originLng?:number;units:Record<string,DeploymentUnit>};
...
export default function WarTheaterMap({attacker,defender,routeType,stage,distanceKm,roundsResolved,deployments=[]}:Props){
 ...
 const fullPath=isLand?landPath:(route?.fullPath||landPath),transit=stage==='ended'?[]:deployments.filter(item=>item.status==='in_transit'||item.status==='preparing'),attackerAtFront=totalFor(deployments,'attacker',['deployed']),defenderAtFront=totalFor(deployments,'defender',['deployed']),defenderPreparing=totalFor(deployments,'defender',['preparing']);
 return <section className="panel war-theater">
  ...
  <div className="war-theater-map">
   <svg viewBox="0 0 1000 500" role="img" aria-label={`War theater from ${attacker.name} to the campaign front in ${defender.name}`}>
    ...
    {transit.map((deployment,index)=>{const originLat=Number(deployment.originLat),originLng=Number(deployment.originLng),hasOrigin=Number.isFinite(originLat)&&Number.isFinite(originLng)&&(originLat!==0||originLng!==0),deploymentPath=hasOrigin?directPath(projectNationLocation(originLat,originLng),destinationPoint):fullPath,marker=interpolate(deploymentPath,deploymentProgress(deployment,now)),offset=(index%3-1)*8,labelLeft=marker.x>920;return <g className={'war-convoy '+deployment.side} transform={`translate(${marker.x} ${marker.y+offset})`} key={deployment.id}><circle r="14"/><Truck x={-8} y={-8} size={16}/><text x={labelLeft?-18:18} y="4" textAnchor={labelLeft?'end':'start'}>R{deployment.arrivesRound}</text><title>{`${deployment.originName||'Homeland'} · arriving round ${deployment.arrivesRound}: ${forceDescription(deployment)}`}</title></g>})}
  ...
  <div className="war-theater-ledger"><div><span className="war-ledger-origin"/><p><b>{attacker.name}</b><small>Homeland · {(attacker.fobs||[]).length} FOB</small></p></div><div><span className="war-ledger-front"/><p><b>{defender.name}</b><small>Campaign front · {(defender.fobs||[]).length} FOB · {attackerAtFront.toLocaleString()} attacking / {defenderAtFront.toLocaleString()} defending units present</small></p></div><div><span className="war-ledger-transit"/><p><b>Mobilizing forces</b><small>Markers advance from their selected deployment origin</small></p></div></div>
 </section>;
}
