...
func (a *app) worldStats(w http.ResponseWriter, r *http.Request) {
	var nations, cities, openOrders, active, active24, activeTwoWeeks, alliances, totalTrades, completedTrades, activeShipments, delayedTrades, activeWars int64
	...
	a.db.QueryRowContext(r.Context(), `SELECT COUNT(*) FROM wars WHERE stage IN('mobilizing','active')`).Scan(&activeWars)
	...
	rows, militaryErr := a.db.QueryContext(r.Context(), `SELECT unit_type,CAST(SUM(quantity) AS SIGNED) FROM (
		SELECT unit_type,quantity FROM military_inventory WHERE unit_type IN('soldiers','tanks','ships','jets','drones')
		UNION ALL
		SELECT resource,escrow_goods FROM market_orders WHERE side='sell' AND status IN('open','pending') AND resource IN('tanks','ships','jets','drones')
		UNION ALL
		SELECT resource,quantity FROM trade_shipments WHERE status IN('in_transit','delayed') AND resource IN('tanks','ships','jets','drones')
	...
	write(w, 200, map[string]any{"nations": nations, "cities": cities, "population": population, "alliances": alliances, "openMarketOrders": openOrders, "activeWars": activeWars, "activePlayers": active, "activePlayers24Hours": active24, "activePlayersTwoWeeks": activeTwoWeeks, "totalTrades": totalTrades, "completedTrades": completedTrades, "activeShipments": activeShipments, "delayedTrades": delayedTrades, "military": military})
}
