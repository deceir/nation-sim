...
func main() {
	...
	bootstrap := []string{
		...
		`INSERT INTO production_quotas(nation_id,commodity,priority) SELECT n.id,q.commodity,q.priority FROM nations n CROSS JOIN (SELECT 'textiles' commodity,14.29 priority UNION ALL SELECT 'processed_foods',14.29 UNION ALL SELECT 'construction_materials',14.29 UNION ALL SELECT 'basic_goods',14.29 UNION ALL SELECT 'consumer_goods',14.28 UNION ALL SELECT 'military_equipment',14.28 UNION ALL SELECT 'luxury_goods',14.28) q WHERE COALESCE((SELECT SUM(x.priority) FROM production_quotas x WHERE x.nation_id=n.id),0)<=0.001 ON DUPLICATE KEY UPDATE priority=VALUES(priority)`,
	}
